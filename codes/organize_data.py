...
def main(file_path: str):
    ...
    if not failed:
        ...
        for k,v in config.items():
            transposed_config.append({"args": k, "values": v})
        table = Tomark.table(transposed_config)
        failed = [float(L_raw_data[i]["test"][-1]["acc"]) < early_stopping_thr for i in [0,1,2,3]]
        md_text = f"# {file_name}\n\n" \
            + (f"### Comment\n\n{config['comment']}\n\n" if config['comment'] else "") \
            + f"### Setting\n\n{table}\n\n" \
            + "***** FAILED *****\n" \
            + f"L_1 failed: {failed[0]}\n" \
            + f"L_2 failed: {failed[1]}\n" \
            + f"L_3 failed: {failed[2]}\n" \
            + f"L_4 failed: {failed[3]}\n"
        f.write(md_text)
        f.close()
